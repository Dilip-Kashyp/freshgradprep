...
const renderEventCard = (event: EventItem, showRsvp: boolean) => (
  <Box
    key={event.name}
    sx={{
      maxWidth: 900,
  ...
  >
    ...
    <Stack spacing={1.5} sx={{ mb: showRsvp ? 4 : 0 }}>
...
const EventsSection = () => {
  return (
    ...
    >
      <Typography variant="h3" sx={{ color: 'primary.main', mb: 6 }}>
        {eventsContent.title}
      </Typography>

      {eventsContent.events.map((event) => renderEventCard(event, true))}

      {eventsContent.pastEvents.length > 0 && (
        <>
          <Typography
            variant="h3"
            sx={{ color: 'primary.main', mt: { xs: 10, md: 14 }, mb: 6 }}
          >
            {eventsContent.pastTitle}
          </Typography>
